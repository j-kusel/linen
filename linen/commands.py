...
try:
    ...
    sys.stderr.write('error in passwords file.')
    sys.stderr.write("do you have passwords.py in the root directory of your Django project?")
    sys.exit(1)
...
env.roledefs.update({
    'local': 'localhost',
    'webserver': S.APACHE_HOSTS,
    'dbserver': S.MYSQL_HOSTS,
    'mediaserver': S.MEDIA_HOSTS,
})
...
def _fetch_git(source):
    if exists(source + '/.git'):
        with cd(source):
            sudo('git pull && git checkout {}'.format(S.REPO_BRANCH))
...
@roles('webserver')
def _install_git():
    sudo('apt-get -y install git-core && git config --global user.name "{}" && git config --global user.email {}'.format(S.USERS['ADMIN_INFO']['name'], S.USERS['ADMIN_INFO']['email']))

@roles('webserver')
def _pull_source(source):
    sudo('git clone {} --branch {} --single-branch {}'.format(S.REPO_URL, S.REPO_BRANCH, source))
...
def _update_settings(source):
    ...
    hosts = S.APACHE_HOSTS
    hosts.append(S.DOMAIN_NAMES)
    hoststr = '", "'.join(hosts)
    sed(settings_path,
        'ALLOWED_HOSTS =.+$',
    ...
    # CHANGE DATABASES
    sql_server = 'localhost' if S.APACHE_HOSTS[0] == S.MYSQL_HOSTS[0] else S.MYSQL_HOSTS[0]
    sudo(r"sed -i.bak -r -e 's/DATABASES = \{/DATABASES = \{'\''default'\'': \{'\''ENGINE'\'': '\''django.db.backends.mysql'\'', '\''NAME'\'': '\''%s_db'\'', '\''USER'\'': '\''%s'\'', '\''PASSWORD'\'': '\''%s'\'', '\''HOST'\'': '\''%s'\'', '\''PORT'\'':'\'''\'',\},/g' %s" % (env.proj, env.user, S.USERS['MYSQL_INFO']['password'], sql_server, settings_path))
...
def _update_virtualenv(source):
    if not exists(env.venv_dir + '/bin/pip'):
        ...
        sudo('virtualenv -p {} {}'.format(S.PYTHON, env.venv_dir))
...
def _config_mysql():
    ...
    sudo("""echo "CREATE USER {0} IDENTIFIED BY '{2}';" | mysql --user=root""".format(env.user, S.MYSQL_HOSTS[0], S.USERS['MYSQL_INFO']['password']))
    sudo("""echo "GRANT ALL ON {0}_db.* TO {1}@{2} IDENTIFIED BY '{3}';" | mysql --user=root""".format(env.proj, env.user, MYSQL_HOSTS[0], S.USERS['MYSQL_INFO']['password']))
    sudo("""echo "GRANT ALL ON {0}_db.* TO {1}@localhost IDENTIFIED BY '{2}';" | mysql --user=root""".format(env.proj, env.user, S.USERS['MYSQL_INFO']['password']))
    ...
    sed(mysql_conf_path, 
        'bind-address.*$',
        'bind-address = {}'.format(APACHE_HOSTS[0]),
        use_sudo=True)
...
def _create_superuser(source):
    with virtualenv(source):
        run("""echo "from django.contrib.auth.models import User; User.objects.create_superuser('{0}', '{1}', '{2}')" | python3 manage.py shell""".format(S.USERS['SUPERUSER']['username'], S.USERS['SUPERUSER']['email'], S.USERS['SUPERUSER']['password']))
